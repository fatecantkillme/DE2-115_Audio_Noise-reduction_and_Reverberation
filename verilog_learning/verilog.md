...
**端口映射方法：端口名关联法：(.底层端口名1(外外接信号名1)，.底层端口名2(外外接信号名2))**   不必按照底层模块端口顺序列表排

### 门原语

调用格式： 门原语名 实例名 （端口连接）

实例名可省，端口连接只能采用顺序法，输出在前输入在后

例子：and (out,in1,in2)

## 数据类型

分为net类（wire）和variable类（**reg**，integer，real，time）

**对于端口信号：input和inout信号必须是net，output可以实reg也可以是wire**
